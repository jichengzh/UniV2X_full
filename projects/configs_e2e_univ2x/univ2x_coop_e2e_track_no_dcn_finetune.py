...
model_other_agent_inf = dict(
    ...
    freeze_bn=True,
    freeze_bev_encoder=True,  # finetune: only train decoder/heads/V2X to recover from DCN removal
    score_thresh=0.4,
...
model_ego_agent = dict(
    ...
    freeze_img_neck=True,
    freeze_bn=True,
    freeze_bev_encoder=True,  # finetune: only train decoder/heads/V2X
    score_thresh=0.4,
    filter_score_thresh=0.35,
    qim_args=dict(
        qim_type="QIMBase",
...
fp16 = dict(loss_scale=512.)  # finetune: enable fp16 to reduce memory (R101+DCN coop OOM at fp32)
...
load_from = "ckpts/univ2x_coop_e2e_stg2_no_dcn.pth"
resume_from = None
