...
model_other_agent_inf = dict(
    ...
    load_from="/home/jichengzhi/UniV2X/projects/work_dirs_e2e_univ2x/univ2x_sub_inf_tiny/epoch_30.pth",
...
model_ego_agent = dict(
    ...
    load_from="/home/jichengzhi/UniV2X/projects/work_dirs_e2e_univ2x/univ2x_sub_vehicle_tiny/epoch_30.pth",
    is_ego_agent=is_ego_agent,
)
...
#                                                        Train&Test Config                                                                                    #
#------------------------------------------------------------------------------------------------------------------#
# Plan B: freeze pretrained, train ONLY V2X fusion (warm-up phase)
#   Fresh fusion modules: lr=1e-3 (high lr to converge fast)
#   Everything else: lr_mult=0 (effectively frozen — AdamW with lr=0 is no-op)
#   Note: weight_decay still applies but with lr=0, decay update is also 0.
#   After 10-15 epochs of warm-up, can unfreeze and do full fine-tune in Plan B2.
optimizer = dict(
    type="AdamW",
    lr=1e-4,
    paramwise_cfg=dict(
        custom_keys={
            "cross_agent_query_interaction": dict(lr_mult=10.0),
            "cross_lane_fusion": dict(lr_mult=10.0),
            "model_ego_agent": dict(lr_mult=0.0),
            "model_other_agent_inf": dict(lr_mult=0.0),
        }
    ),
    weight_decay=0.01,
)
