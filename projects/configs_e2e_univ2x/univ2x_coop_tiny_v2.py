...
#                                                        Train&Test Config                                                                                    #
#------------------------------------------------------------------------------------------------------------------#
# Plan C: differential learning rates
#   Fresh V2X fusion (16 keys): lr_mult=10 -> 1e-3 (converge fast)
#   Pretrained ego_agent (~1300 keys): lr_mult=0.1 -> 1e-5 (preserve features)
#   img_backbone: lr_mult=0.01 -> 1e-6 (most fragile, lowest lr)
#   mmcv matches LONGEST substring first, so specific keys win.
optimizer = dict(
    type="AdamW",
    lr=1e-4,
    paramwise_cfg=dict(
        custom_keys={
            ...
            "model_ego_agent.img_backbone": dict(lr_mult=0.01),
            "model_ego_agent": dict(lr_mult=0.1),
        }
    ),
    weight_decay=0.01,
)
