#!/usr/bin/env python3
"""Run or import a Stage2 true-FP16 AP eval for one original60 label."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--precision-mode",
        choices=("model_half", "amp_fp16"),
        default="model_half",
        help="model_half casts model and floating batch tensors to half; amp_fp16 uses autocast.",
    )
    return parser.parse_args()


def resolve_cli_paths(args: argparse.Namespace, launch_cwd: Path) -> None:
    for attr in ("ckpt_dir", "raw_dir", "rows_out", "report_json", "heal_root"):
...
def run_true_fp16_eval(args: argparse.Namespace, report_path: Path) -> dict[str, Any]:
    import importlib
    ...
    if args.precision_mode == "model_half":
        model = model.half()
    model.eval()
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if args.precision_mode == "model_half":
                batch_data = cast_floating_tensors(batch_data, torch.float16)
                ...
                infer_result = inference_intermediate_fusion_fp32_postprocess(
                    ...
                    model,
                    dataset,
                    model_context=torch.cuda.amp.autocast(dtype=torch.float16),
                )
    ...
    ap30, ap50, ap70 = eval_utils.eval_final_results(
        result_stat,
        str(raw_dir),
        f"{args.label}_{args.precision_mode}",
    )
    report = {
        "status": "success",
        "label": args.label,
    ...
    write_json(
        ...
        {
            "schema": "stage2_true_fp16_ap_precision_summary_v1",
            "precision_mode": args.precision_mode,
            "model_dtype_counts": before_counts,
            "runner": (
                "model.half plus recursive floating tensor half cast"
                if args.precision_mode == "model_half"
                else "torch.cuda.amp.autocast(dtype=float16)"
            ),
...
def build_row(args: argparse.Namespace, report: dict[str, Any], report_path: Path) -> dict[str, Any]:
    ...
    ckpt_dir = Path(args.ckpt_dir)
    raw_dir = Path(args.raw_dir)
    ckpt_path = Path(str(report.get("ckpt_path") or best_checkpoint(ckpt_dir)))
    ...
    if not layer_summary.exists():
        write_json(
            ...
            {
                "schema": "stage2_true_fp16_ap_precision_summary_v1",
                "precision_mode": report.get("precision_mode", "model_half"),
                "runner": "true_fp16 report import",
                "full_network_claim": False,
            },
        )
    config_id = stable_config_id(
        model="pyramid_lidar",
        candidate_id=f"original60:{label}:fp16:ap",
        software_point_id=f"true_fp16_original60:{label}:{'x'.join(str(item) for item in width)}:fp16:ap",
        quant_policy="fp16",
    ...
    row = ap_anchor_row(
        ...
        candidate_id=f"original60:pyramid_lidar:{label}:fp16",
        software_point_id=f"true_fp16_original60:{label}:{'x'.join(str(item) for item in width)}:fp16:ap",
        dense_stage="model",
        ...
        finetune_protocol="structural_prune_pyramid_train_ddp_half_true_fp16_model_eval",
        training_budget="existing_stage2_original60_s0_024_or_label_checkpoint",
        eval_command=str(report.get("eval_command") or "stage2_h800_true_fp16_ap_eval.py"),
        provenance="stage2_original60_true_fp16_h800_model_eval",
        run_id=args.run_id or f"true_fp16_original60_ap_{label}",
        created_at=args.created_at or utc_timestamp(),
        source_files=source_files,
        raw_artifact=str(raw_dir),
        notes="true_fp16 model_eval AP row from measured report",
        precision="fp16",
        quant_scheme="fp16_model_eval",
...
def main() -> int:
    ...
    try:
        if args.execute:
            ...
            report = run_true_fp16_eval(args, report_path)
        else:
