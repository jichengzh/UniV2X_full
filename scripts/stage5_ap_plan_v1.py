#!/usr/bin/env python3
"""Build numerical-sanity and full-AP jobs for a Stage5 feedback round."""
...
SOURCE_SCHEMA = "stage5_performance_manifest_v1"
SCHEMA_VERSION = "stage5_ap_plan_v1"


def build_stage5_ap_plan(
    ...
    *,
    ...
    expected_groups = int(manifest.get("group_count", -1))
    if expected_rows <= 0 or expected_rows != expected_groups * 4:
        raise ValueError("Stage5 AP input must contain complete four-arm groups")
...
def main() -> int:
    ...
    print(json.dumps({"schema_version": SCHEMA_VERSION, "row_count": len(rows), "terminal_counts": counts}, sort_keys=True))
