#!/usr/bin/env python3
"""Launch CUTLASS-BYOC FP16 latency+energy jobs for v2 cold-start 96补点."""
...
OUT_ROOT = REPO / "results/v2_gold_coldstart_96_20260708/cutlass_energy_raw"
READY_WIDTHS = (
    "16x32x64",
    "24x32x96",
    "32x32x128",
    ...
    return [
        {
            ...
            "out_json": str(OUT_ROOT / width / f"cutlass_fp16_energy_gpu{gpu}.json"),
        }
        for width in widths
    ]
...
def command_for(job: dict[str, Any]) -> list[str]:
    return [
        str(TVM_PYTHON),
        "scripts/stage2_codriving_cutlass_byoc_probe_v1.py",
        "--gpu",
        str(job["gpu"]),
        "--onnx",
        str(job["onnx"]),
...
def main() -> int:
    ...
    for job in jobs:
        ...
        print(json.dumps({"width": job["width"], "status": status["status"]}, sort_keys=True), flush=True)
    return 0
