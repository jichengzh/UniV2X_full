"""L4 DELIVERABLE B2: W_g AP70 for rank-flip pairs 4/5/6 on GPU 1.

Configs (all wpg=4 to match base model):
  wg_pair4: [48, 96, 256]  W_g for pair4
  wg_pair5: [48, 32, 128]  W_g for pair5
  wg_pair6: [48, 64, 192]  W_g for pair6

Runs serially on GPU 1. Updates results/l4_b2_ap_finetune.json.
...
Usage:
  ...
  /home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python \
    scripts/phase2/l4_wgpair_runner.py \
    > results/l4_wgpair_chain.log 2>&1 &
"""
...
def run_one(tag, nf, results):
    ...
    if not validate_config(nf):
        results[tag] = {"num_filters": nf, "wpg": WPG, "groups": GROUPS,
                        "ap70": None, "ap50": None,
                        "status": f"infeasible:width=0"}
        save_results(results)
        return

    out_dir = prune(tag, nf)
    if out_dir is None:
        results[tag] = {"num_filters": nf, "wpg": WPG, "groups": GROUPS,
...
def main():
    print("=== L4 WG-Pair Runner (GPU 1) ===")
    results = load_results()
    for tag, nf in CONFIGS:
        run_one(tag, nf, results)
    print("\n=== DONE ===")
    print(json.dumps({k: results[k] for k in results if k.startswith("wg_pair")}, indent=2))
