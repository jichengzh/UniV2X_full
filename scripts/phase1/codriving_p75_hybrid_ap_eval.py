"""CoDriving p75 hybrid AP eval harness — DAIR val 1789.

Architecture: PyTorch VFE + scatter (p75 ckpt) -> TRT p75 collab engine -> PyTorch postprocess.

p75 model: backbone pruned 90.7%, backbone+heads 705K params.
p75 collab TRT engine: processes (2,64,256,512) spatial_features + pairwise_t_matrix.
Outputs cls_preds (1,1,128,256) + reg_preds (1,8,128,256).

FP16 sanity target: AP50 ≈ 0.618 (PyTorch p75 baseline from H800 eval).
FP16 sanity gate: |ΔAP50| < 0.02 vs target.

Usage:
    # FP16 sanity gate
    CUDA_VISIBLE_DEVICES=2 python scripts/phase1/codriving_p75_hybrid_ap_eval.py \
        --engine output/codriving_pilot/collab_engines/codriving_collab_p75_fp16.engine \
        --tag p75_fp16 --n-samples 1789

    # INT8 eval
    CUDA_VISIBLE_DEVICES=2 python scripts/phase1/codriving_p75_hybrid_ap_eval.py \
        --engine output/codriving_pilot/collab_engines/codriving_collab_p75_int8.engine \
        --tag p75_int8 --n-samples 1789
"""
...
# p75 config and ckpt
HYPES_PATH = REPO_ROOT / "output/codriving_pilot/p75/config_finetune.yaml"
CKPT_PATH  = REPO_ROOT / "output/codriving_pilot/p75_warmstart/net_epoch_bestval_at5.pth"
...
def run_eval(args):
    ...
    print(f"[p75 eval] Loading p75 config: {HYPES_PATH}")
    ...
    print(f"[p75 eval] Creating model (p75 arch: num_filters=[64,32,64])")
    model = train_utils.create_model(hypes)

    print(f"[p75 eval] Loading p75 ckpt: {CKPT_PATH}")
    ...
    print(f"[p75 eval] Building dataset (DAIR val)...")
    ...
    if not args.force_fallback:
        print(f"[p75 eval] Loading TRT engine: {args.engine}")
    ...
    print(f"[p75 eval] Running {args.n_samples} samples (tag={args.tag})...")
    ...
    elapsed = time.time() - t_start
    print(f"\n[p75 eval] Done: n={n_done} trt={n_trt_path} fb={n_fallback_path} skip={n_skipped}")
    ...
    print(f"  (sanity target p75 FP16: AP50≈0.618)")
    ...
    row = {
        ...
        "note": f"DAIR_val_1789_p75_{args.tag}_hybrid_pytorch_trt",
    }
    ...
    with open(str(RESULTS_CSV), "a", newline="") as f:
        ...
        if write_header:
            writer.writeheader()
        writer.writerow(row)
    print(f"\n[p75 eval] Result appended to: {RESULTS_CSV}")
