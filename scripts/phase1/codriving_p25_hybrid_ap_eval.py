"""CoDriving p25 hybrid AP eval harness — DAIR val 1789.

Architecture: PyTorch VFE + scatter (p25 ckpt) -> TRT p25 collab engine -> PyTorch postprocess.

p25 model: backbone pruned 44.75%, 4.19M params. num_filters=[32,96,192].
p25 collab TRT engine: processes (2,64,256,512) spatial_features + pairwise_t_matrix.
Outputs cls_preds (1,1,128,256) + reg_preds (1,8,128,256).

FP16 sanity target: AP50 ≈ 0.587 (PyTorch p25 baseline from H800 eval, 1789 frames).
FP16 sanity gate: |ΔAP50| < 0.02 vs target.

Usage:
    # FP16 sanity gate
    CUDA_VISIBLE_DEVICES=7 python scripts/phase1/codriving_p25_hybrid_ap_eval.py \
        --engine /home/jichengzhi/V2X/output/codriving_pilot/collab_engines/codriving_collab_p25_fp16_precise.engine \
        --tag p25_fp16 --n-samples 1789

    # INT8 eval
    CUDA_VISIBLE_DEVICES=7 python scripts/phase1/codriving_p25_hybrid_ap_eval.py \
        --engine /home/jichengzhi/V2X/output/codriving_pilot/collab_engines/codriving_collab_p25_int8_precise.engine \
        --tag p25_int8 --n-samples 1789
"""
...
# p25 config and ckpt
HYPES_PATH = REPO_ROOT / "output/codriving_pilot/p25/config_finetune.yaml"
CKPT_PATH  = REPO_ROOT / "output/codriving_pilot/p25_warmstart/net_epoch_bestval_at5.pth"
...
class TrtCollabCodriving:
    def __init__(self, engine_path: str):
        ...
        if self.ctx is None:
            raise RuntimeError(f"TRT create_execution_context() returned None for {engine_path}.")
    ...
    def __call__(self, spatial: torch.Tensor, pairwise_t_matrix: torch.Tensor) -> dict:
        ctx = self.ctx
        ...
        return {"cls_preds_raw": bufs[cls_name], "reg_preds_raw": bufs[reg_name]}
...
def run_eval(args):
    ...
    print(f"[p25 eval] Loading p25 config: {HYPES_PATH}")
    ...
    print(f"[p25 eval] Creating model (p25 arch: num_filters=[32,96,192])")
    model = train_utils.create_model(hypes)

    print(f"[p25 eval] Loading p25 ckpt: {CKPT_PATH}")
    ...
    print(f"[p25 eval] Building dataset (DAIR val)...")
    ...
    if not args.force_fallback:
        print(f"[p25 eval] Loading TRT engine: {args.engine}")
    ...
    print(f"[p25 eval] Running {args.n_samples} samples (tag={args.tag})...")
    with torch.inference_mode():
        for batch_data in loader:
            ...
            if args.force_fallback or trt_engine is None or n_agents != 2:
                ...
                pairwise_t_matrix = ego["pairwise_t_matrix"]

                trt_out = trt_engine(spatial_features, pairwise_t_matrix)
                cls_raw = trt_out["cls_preds_raw"]
                reg_raw = trt_out["reg_preds_raw"]
            ...
            for iou_th in (0.3, 0.5, 0.7):
                eval_utils.caluclate_tp_fp(
                    pred_box_tensor, pred_score, gt_box_tensor, result_stat, iou_th)
    ...
    elapsed = time.time() - t_start
    print(f"\n[p25 eval] Done: n={n_done} trt={n_trt_path} fb={n_fallback_path} skip={n_skipped}")

    out_dir = REPO_ROOT / f"results/E_codriving_{args.tag}_p25"
    out_dir.mkdir(parents=True, exist_ok=True)
    ap_30, ap_50, ap_70 = eval_utils.eval_final_results(result_stat, str(out_dir))

    print(f"  AP30={ap_30:.4f}  AP50={ap_50:.4f}  AP70={ap_70:.4f}")
    print(f"  (sanity target p25 FP16: AP50≈0.587)")

    sanity_target = 0.587
    if "fp16" in args.tag:
        delta = abs(float(ap_50) - sanity_target)
        if delta < 0.02:
    ...
    cols = ["tag", "model", "precision", "device", "gpu", "n_samples", "n_trt", "n_fb",
            "ap30", "ap50", "ap70", "elapsed_secs", "note"]
    row = {
        "tag": args.tag,
        "model": "codriving_collab_p25",
        "precision": "fp16" if "fp16" in args.tag else "int8",
        "device": "RTX_4090",
        "gpu": f"GPU{os.environ.get('CUDA_VISIBLE_DEVICES', '?')}",
        "n_samples": args.n_samples,
        "n_trt": n_trt_path,
        "n_fb": n_fallback_path,
        "ap30": round(float(ap_30), 4),
        "ap50": round(float(ap_50), 4),
        "ap70": round(float(ap_70), 4),
        "elapsed_secs": round(elapsed, 1),
        "note": f"DAIR_val_1789_p25_{args.tag}_hybrid_pytorch_trt",
    }
    ...
    with open(str(RESULTS_CSV), "a", newline="") as f:
        ...
        if write_header:
            writer.writeheader()
        writer.writerow(row)
    print(f"\n[p25 eval] Result appended to: {RESULTS_CSV}")
    print(f"  Row: ap30={row['ap30']}  ap50={row['ap50']}  ap70={row['ap70']}")
