#!/usr/bin/env python3
"""Build CoDriving TVM/TRT Stage6 paper tables at frozen AP-loss thresholds."""
...
from framework.stage6.paper_table_v1 import build_backend_tables  # noqa: E402
...
FIELDS = (
    ...
    "ap_constraint_violated", "latency_rank", "evidence_origin",
)
...
def _markdown(rows: list[dict[str, Any]], backend: str, delta: float, floor: float) -> str:
    lines = [
        f"# CoDriving Stage6 {backend.upper()} main table (DeltaAP_max={delta:.2f})",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--delta-ap-max", type=float, nargs="+", default=(0.05, 0.10))
    ...
    for backend in ("tvm", "trt"):
        ...
        for key, rows in result["tables"].items():
            ...
            (args.output_dir / f"{stem}.md").write_text(
                _markdown(rows, backend, delta, result["baseline_ap70"] - delta),
                encoding="utf-8",
            )
    audit = {
        "schema_version": "stage6_codriving_paper_main_table_audit_v1",
        ...
        "evidence_provenance_legend": {
            "stage6_new_original_default_measurement": "Arm 1 measured in this Stage6 run",
            "stage6_new_control_measurement": "Arm 2-5 control measured in this Stage6 run",
            "joint_actual_v3_initial_observed_gold176": "Arm 6 initial observed point admitted from Gold176",
            "joint_actual_v3_online_feedback": "Arm 6 online point from the closed actual-feedback v3 trajectory",
        },
        "backends": outputs,
    }
    (args.output_dir / "stage6_codriving_paper_main_table_audit_v1.json").write_text(
