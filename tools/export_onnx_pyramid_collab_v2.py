...
def manual_affine_grid(M: torch.Tensor, dsize, align_corners=False):
    ...
    # einsum-free equivalent (relax onnx Einsum mis-infers nij,hwj->nhwi):
    #   grid[n,h,w,i] = sum_j M[n,i,j] * base[h,w,j]   via bmm
    N = M.shape[0]
    base_flat = base.reshape(-1, 3)                           # (H*W, 3)
    base_b = base_flat.unsqueeze(0).expand(N, -1, -1)         # (N, H*W, 3)
    grid = torch.bmm(base_b, M.transpose(1, 2))               # (N, H*W, 2)
...
def parse_args():
    ...
    p.add_argument("--out", default=str(REPO_ROOT / "models/stage_a_cache/base_v2.onnx"))
