...
\usepackage{booktabs}
\usepackage{amsmath} % for \text in math mode (Method section)
...
%==================== METHOD SECTION (inserted) ====================
\section{Method}

We cast software--hardware co-optimization of cooperative V2X perception
as a two-stage pipeline (Figure~\ref{fig:framework}). Stage~1 performs a
one-shot, deployment-oriented co-characterization that translates a
\emph{theoretically} configurable network into \emph{concrete} search
variables and their feasible ranges, given a target accelerator. Stage~2
then jointly searches the resulting
pruning\,$\times$\,quantization\,$\times$\,schedule space with a nested
double-loop optimizer driven by a hierarchical cost model, and returns a
regime-conditioned Pareto frontier of deployment plans.

\begin{figure}[t]
\centering
\framebox[0.95\columnwidth][c]{\rule{0pt}{3.2cm}\parbox{0.9\columnwidth}{\centering\small
\emph{[Framework overview placeholder]}\\[2pt]
Stage~1 (capability scan $\mathcal{H}$ $+$ DepGraph partition $\to$ manifest)
$\;\to\;$ Stage~2 (nested double-loop search: outer NSGA-II over
$P\times Q$ $\times$ inner schedule autotuning, hierarchical cost model)
$\;\to\;$ regime-conditioned Pareto frontier $\to$ deploy plan.}}
\caption{The two-stage software--hardware co-optimization framework.}
\label{fig:framework}
\end{figure}

\subsection{Stage 1: Network--Hardware Co-Characterization}

The set of legal configurations is determined jointly by network
structure and target accelerator: a pruning ratio can attach only to
channel groups that the structure forces to be removed together; a
module's feasible quantization granularity depends on the hardware IP it
maps to; and the device-routing space is bounded by the operator
whitelist. Stage~1 therefore runs two parallel scans whose products merge
into a \emph{partition manifest}.

\noindent\textbf{Hardware capability scan.}
We abstract each accelerator into a machine-readable descriptor
$\mathcal{H}=\langle\mathcal{I},\mathbf{a},\mathcal{Q},\mathcal{T}\rangle$:
available compute IPs $\mathcal{I}$ (GPU, DLA core, vendor NPU) with their
supported precisions and operator whitelists; tensor-core channel-alignment
requirements $\mathbf{a}$ (e.g.\ $a_{\mathrm{int8}}{=}32$,
$a_{\mathrm{fp16}}{=}8$) with their enforcement mode; quantization
constraints $\mathcal{Q}$; and a pinned toolchain $\mathcal{T}$ for
reproducibility. The descriptor is \emph{capability-driven} rather than
device-specific: when a target lacks a DLA, the routing axis automatically
degenerates to GPU-only and every DLA-induced constraint is dropped, so the
same procedure specializes to heterogeneous hardware unchanged.

\noindent\textbf{Computation-graph dependency partition.}
A thin per-model trace adapter isolates the dense, channel-coupled
core---the only part amenable to structured pruning and static
tracing---excluding the sparse encoder (voxelization, Lift-Splat-Shoot)
and bypassing the channel-preserving cooperative-fusion neck. On this core,
DepGraph operator-coupling analysis~\cite{fang2023depgraph} builds a
dependency graph and derives three views on a single graph:
(B1)~\emph{pruning groups}, each a minimal jointly-prunable unit annotated
with its width $w_g$, a hardware-derived alignment floor, and a maximum
ratio $r^{\max}_g$; (B2)~\emph{quantization units}, each a union of complete
B1 groups so that a quantization boundary never splits a coupled group; and
(D)~per-operator \emph{routing} annotation of DLA reachability. Grouping
follows dataflow coupling, not module names, recovering cross-module
dependencies that name-based partitioning misses (e.g.\ a backbone stem
forced to prune jointly with the first BEV residual stage).

\noindent\textbf{Consolidation and validation.}
Treating each group as a free variable yields an unenumerable space
($\sim 6^{43}$ for our largest model). A consolidation layer binds groups
sharing a semantic bucket and stage index into one pruning-ratio knob---which
inherits the tightest member bound and always stays legal since binding only
removes degrees of freedom---and folds the per-operator routing annotation
into a few maximal contiguous routable segments, the true decision
granularity because every GPU$\leftrightarrow$DLA switch incurs a reformat
cost. This compresses the space from $\sim 10^{33}$ to $\sim 10^{4}$ while
retaining the structural view as ground truth. We further attach a
\emph{descriptive} baseline-latency profile---per-operator CUDA-event timing
aggregated back onto each knob along the same layer names~\cite{yang2018netadapt}---so
that Stage~2 can steer compression budget toward high-latency units; this is
an input prior, not a legality constraint. Each manifest is checked by a
fail-fast trial prune (ratio $0.5$ plus one forward pass).

\subsection{Stage 2: Software--Hardware Co-Optimization}

\noindent\textbf{Why joint rather than serial.}
Two reasons forbid optimizing the axes independently, or in a
``prune-then-schedule'' pipeline. \emph{(i)~Cross-dimension coupling trap}:
a local optimum on one axis is cancelled on another---e.g.\ pruning a module
to a non-aligned width makes its grouped convolution's
\texttt{in\_per\_group} miss the tensor-core fast kernel and fall back to a
slow kernel, so \emph{more} aggressive pruning becomes \emph{slower}.
\emph{(ii)~Structural local optimum of serial greedy} (the deeper reason):
a serial searcher that first fixes the pruning width can observe each width's
latency only under the \emph{default} schedule, and thus greedily locks the
width $W_g$ that is optimal under that lens. But schedule-tuning headroom is
\emph{uneven} across widths---changing the width rebuilds the legal tile
factors, the reachable fast kernels, and the fusibility---so a width $P_g$
that is mediocre by default may, once tuned by the schedule
autotuner, overtake $W_g$. Having locked $W_g$, the serial pipeline never
revisits the width, and the $P_g$ branch---single-axis sub-optimal yet
globally optimal after joint tuning---is structurally excised from its
reachable space. Hence all axes must be solved within one searcher.

\noindent\textbf{Search-space construction.}
A candidate is a triple $c=\langle B_1,B_2,D\rangle$ whose induced width $W$
and number format $P$ in turn induce a schedule sub-space
$\Sigma_{\mathrm{sch}}(W,P)$; the joint scale is
$\sim\!10^{15}\times O(10^{9})$ and explodes combinatorially. We render it
enumerable through three mechanisms rather than arbitrary truncation. A
\emph{two-stage definition}~\cite{xu2023alt} makes the inner schedule space
be \emph{rebuilt} for each selected $(W,P)$ rather than a fixed Cartesian
product, so ``joint'' is literal: the schedule space is a \emph{function} of
the software configuration. \emph{Block decomposition}~\cite{shu2021chanas}
pre-tunes each $(\text{block},W,P)$ once offline into a schedule lookup table
(LUT) of \emph{linear} cost $B\cdot|W|\cdot|P|$ instead of
$(|W|\,|P|\,|S|)^{B}$, after which the outer loop composes latency by table
lookup. \emph{Legality predicates with dependency propagation} prune illegal
configurations analytically: a grouped-conv layer hits the fast kernel iff
$\mathrm{in\_per\_g}(W)\in\{2^{k}\}$, sensitive units (heads, attention,
\texttt{grid\_sample}) are gated to FP16, and a DLA-routed module is forced
to INT8\,$\wedge$\,per-tensor. Collapsing the pruning object to
\texttt{channel} (semi-/unstructured variants yield no real speedup on the
target hardware) leaves a $\sim\!10^{4}$ software space plus a linear LUT.

\noindent\textbf{Hierarchical cost model.}
The searcher is driven by an evaluator that avoids a real measurement per
candidate and is a \emph{hierarchical, heterogeneous} ensemble rather than a
single regressor---a choice forced by our being multi-objective with a
non-purely-predictable accuracy, unlike prior single-objective latency
tuners. The inner (schedule) model reuses the autotuner's XGBoost cost model
over low-level AST features~\cite{chen2018learning}; the outer model is a
per-$(\text{model},\text{hardware})$ gradient-boosted tree (LightGBM) whose
splits naturally fit non-smooth kernel-alignment cliffs and conditional
features. Latency and energy use a \emph{learning-to-rank} head---grouped by
$(\text{model},\text{hw})$ so cross-magnitude pairs never mix, which is
immune to the cross-magnitude collapse that crashed naive regressors---plus a
$\log$-regression head supplying Pareto coordinates. Accuracy is handled by a
\emph{funnel}: a $\Delta\mathrm{AP}$ residual predictor cheaply ranks
candidates, but precise AP comes only from finetuned, full-frame anchors fed
back into retraining---AP is never a pure prediction on the final frontier. A
feasibility classifier gates build failure and accuracy crash before any
regression, and cross-hardware transfer uses a global$+$local decomposition
$\hat f_{\mathrm{Orin}}(c)=\hat f^{\mathrm{global}}(c)+\hat
f^{\mathrm{local}}_{\mathrm{Orin}}(c)$, where the global head is fit on
abundant 4090 samples and the local head on a few on-device measurements.

\noindent\textbf{Regime-conditioned Pareto and the searcher.}
The evaluator emits five perception metrics
$(\mathrm{AP},\ell,\tau,e,s)$---accuracy, latency, throughput, energy, model
size---and two closed-loop driving metrics $(\mathrm{DS},\mathrm{RC})$,
driving score and route completion. ``Better'' is decided by a
multi-objective Pareto frontier, since no scalar score can rank
configurations that trade accuracy against latency and energy. A selector
partitions the seven metrics into an objective set $\mathcal{O}_\rho$ and a
constraint set $\mathcal{S}_\rho$ per deployment regime $\rho$
(ego vehicle, roadside unit, energy-sensitive, driving-safety), computing
dominance only on $\mathcal{O}_\rho$ and feasibility only on
$\mathcal{S}_\rho$; \textbf{accuracy always remains an objective and is never
demoted to a constraint}, since how much accuracy buys how much speedup is
the central question. The same predictions serve every regime without
retraining the evaluator. The frontier is found by a nested double loop: an
outer multi-objective NSGA-II~\cite{deb2002nsga} over the software axes
$P\times Q$, where each candidate first consults the schedule LUT (a miss
triggers the inner schedule search and is written back), composed with a
\emph{topology-aware} latency model that switches among additive (serial
blocks), $\max$+handoff (heterogeneous GPU\,$\parallel$\,DLA), and
whole-segment (fused cooperative join) terms instead of blind summation. An
active-learning loop---acquisition weighted by quantile uncertainty
$\times$ frontier proximity, plus an $\varepsilon$-greedy reserve---selects
the next $K{=}8\text{--}16$ candidates for real measurement and retraining.
The frontier's Top-$K$ plans are passed on for full-network verification and
engine compilation.
