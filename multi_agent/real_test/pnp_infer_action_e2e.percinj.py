...
def turn_traffic_into_map(all_bbox, det_range):
    ...
    for idx in range(1):
        ...
        # compat: tostring_rgb removed in matplotlib>=3.8; use buffer_rgba fallback
...
class PnP_infer():
	def __init__(self, config=None, ego_vehicles_num=1, perception_model=None, planning_model=None, perception_dataloader=None, device=None) -> None:
		...
		############
		###### latency-aware frame alignment (sim_closedloop_arch_v1.md §3.2/§3.3)
		############
		# Dynamic latency->frame delay + ZOH for the RSU branch. Driven by either:
		#   (a) 'latency_inject_ms': scalar constant (injected E7 archive)
		#   (b) 'latency_trace': path to CSV produced by collect_rsu_latency_trace.py
		#                        (trace-replay, §3.8 R-2/R-3)
		#   (c) 'latency_aware: true': per-frame measured in-carla timing
		# Options (a) and (b) are mutually exclusive.  See design §3.4/§3.8.
		self._lat_aligner = None
		self._lat_audit_rows = []
		self._latency_inject_ms = None
		self._trace_reader = None           # ★ R-3: populated for latency_trace mode
		self._latency_source_tag = 'measured_in_carla'
		sim_cfg = self.config.get('simulation', {})
		if 'latency_inject_ms' in sim_cfg and 'latency_trace' in sim_cfg:
			raise ValueError(
				"pnp_infer: simulation.latency_inject_ms and simulation.latency_trace "
				"are mutually exclusive — specify only one.")
		if 'latency_trace' in sim_cfg:
			# ★ R-3 trace-replay mode (§3.8)
			from team_code.closedloop.latency_frame_align import (
				RsuLatencyAligner, LatencyTraceReader)
			self._lat_aligner = RsuLatencyAligner()
			self._trace_reader = LatencyTraceReader(sim_cfg['latency_trace'])
			self._latency_source_tag = LatencyTraceReader.LATENCY_SOURCE
			print("[pnp_infer] latency_trace mode: %s (%d entries)" %
				  (sim_cfg['latency_trace'], self._trace_reader.n_calls))
		elif 'latency_inject_ms' in sim_cfg:
			from team_code.closedloop.latency_frame_align import RsuLatencyAligner
			self._lat_aligner = RsuLatencyAligner()
			self._latency_inject_ms = float(sim_cfg['latency_inject_ms'])
			self._latency_source_tag = 'injected_from_E7'
		elif sim_cfg.get('latency_aware', False):
			from team_code.closedloop.latency_frame_align import RsuLatencyAligner
			self._lat_aligner = RsuLatencyAligner()
			self._latency_source_tag = 'measured_in_carla'

		############
		###### ★ ego 感知推理延迟 (τ_perc): 延迟 ego 自身输入 → 真实推理延迟建模
		############
		self._tau_perc_ms = float(sim_cfg.get('tau_perc_ms', 0.0))
		self._ego_perc_bank = {}
		if self._tau_perc_ms > 0:
			print("[pnp_infer] ★ τ_perc=%.0fms → Δ_perc=%d frames (ego 感知延迟)" % (self._tau_perc_ms, math.ceil(self._tau_perc_ms/50.0)))

		############
		###### ★ I-1 L1 trajectory tracking plan store
		############
		# When simulation.l1_control: true, non-inference frames use L1 (world-frame
		# waypoint anchor + time-indexed speed) instead of L0 (frozen prev_control).
		# One L1PlanStore per ego vehicle; updated each inference frame in
		# generate_action_from_model_output(); read each tick in run_l1_step().
		self._l1_plan_stores = None   # None → L0 path (backward compatible)
		self._l1_controllers = None   # ★ Fix-D: isolated PID instances for L1 ticks
		if sim_cfg.get('l1_control', False):
			from team_code.closedloop.l1_trajectory_controller import L1PlanStore
			# ★ I-2 τ_ego injection: ego self-inference latency → plan availability delay.
			#   tau_ego_ms = 0 (default) → no delay → byte-identical to I-1 baseline.
			#   tick_dt = 1/20s = 50ms (CARLA sync mode); Δ_ego = ceil(tau_ego_ms/50).
			self._tau_ego_ms = float(sim_cfg.get('tau_ego_ms', 0.0))
			_tau_ego_s = self._tau_ego_ms / 1000.0
			self._l1_plan_stores = [L1PlanStore(tau_ego_s=_tau_ego_s)
			                        for _ in range(self.ego_vehicles_num)]
			if self._tau_ego_ms > 0:
				import math as _math_l1
				print("[pnp_infer] ★ I-2 τ_ego=%.0fms → Δ_ego=%d frames (ego plan delayed)"
				      % (self._tau_ego_ms, _math_l1.ceil(self._tau_ego_ms / 50.0)))
			# ★ Fix-D: create SEPARATE V2X_Controller instances for L1 non-inference ticks.
			# Rationale: V2X_Controller.run_step() mutates 4 rate-sensitive state fields:
			#   turn_controller._window (deque, n=5)  — integral/derivative, 5Hz→1s, 20Hz→0.25s
			#   speed_controller._window (deque, n=20) — integral/derivative
			#   stop_steps  — increments when speed<0.2; triggers forced_forward at 100
			#   forced_forward_steps — 12-step countdown at throttle=0.8
			# Sharing these with inference-frame controller (called at 5Hz) causes:
			#   stop_steps accumulates 4× faster → forced_forward at bends → coll_lay
			#   PID integral corrupted by 20Hz sampling (window covers 0.25s not 1s)
			# Isolation: inference frames keep self.controller[ego_i] (5Hz, correct);
			#            L1 frames use self._l1_controllers[ego_i] (own PID, 15Hz, consistent).
			self._l1_controllers = [V2X_Controller(self.config['control'])
			                        for _ in range(self.ego_vehicles_num)]
			print("[pnp_infer] ★ L1 trajectory tracking enabled (%d ego vehicle(s))"
				  % self.ego_vehicles_num)
			print("[pnp_infer] ★ Fix-D: isolated L1 controller instances created")

		############
		###### τ-trace collection (§3.8 R-1: measure real per-call perception τ)
		############
		# When simulation.trace_collect_out is set (R-1 collect run), the perception
		# forward is wrapped with CUDA-Event timing each frame.  Warmup calls
		# (simulation.trace_collect_warmup, default=20) are excluded from the CSV.
		# 口径: CUDA-Event around to_device→post_process (forward+postproc,
		#        excl voxelize/IPC). NOT "forward-only" — includes NMS/post_process.
		# latency_ms_source = 'trace_replay_codriving_4090' (same tag as trace mode).
		self._trace_collect_out    = sim_cfg.get('trace_collect_out', None)
		self._trace_collect_warmup = int(sim_cfg.get('trace_collect_warmup', 20))
		self._trace_collect_calls  = 0        # total calls (warmup included)
		self._trace_collect_rows   = []       # list of (call_idx, tau_ms)
		if self._trace_collect_out is not None:
			import os as _os
			_os.makedirs(_os.path.dirname(_os.path.abspath(self._trace_collect_out)),
						 exist_ok=True)
			try:
				self._tc_evt_s = torch.cuda.Event(enable_timing=True)
				self._tc_evt_e = torch.cuda.Event(enable_timing=True)
				self._tc_cuda  = True
			except Exception:
				self._tc_cuda  = False
			print("[pnp_infer] ★ trace_collect_out=%s  warmup=%d  cuda_event=%s" %
				  (self._trace_collect_out, self._trace_collect_warmup,
				   getattr(self, '_tc_cuda', False)))
	...
	def _dump_latency_audit(self):
		"""Write the per-frame latency-alignment audit log (design §3.3-3, 6 cols).
		CSV columns: step,delta,used_rsu_frame_id,is_zoh_held,latency_ms_source,zoh_age_frames.
		Written under save_path/meta if available, else /tmp."""
		import csv as _csv
		if self.save_path is not None:
			out = os.path.join(str(self.save_path), 'meta', 'latency_align_audit.csv')
		else:
			out = '/tmp/latency_align_audit.csv'
		try:
			with open(out, 'w', newline='') as f:
				w = _csv.DictWriter(f, fieldnames=[
					'step', 'delta', 'used_rsu_frame_id', 'is_zoh_held',
					'latency_ms_source', 'zoh_age_frames'])
				w.writeheader()
				for row in self._lat_audit_rows:
					w.writerow(row)
		except Exception as ex:
			print('latency audit dump failed:', ex)

	def _flush_trace_collect(self):
		"""Append all accumulated (call_idx, tau_ms) rows to trace_collect_out CSV.

		Called after every timed forward pass so the trace is persisted
		incrementally (survives SIGKILL mid-run).  §3.8 R-1.
		口径: CUDA-Event around to_device → post_process (forward+postproc,
		excl voxelize/IPC). NOT "forward-only" — includes NMS/post_process.
		"""
		import csv as _csv
		if not self._trace_collect_out or not self._trace_collect_rows:
			return
		# write the entire accumulated list (re-write) to keep the file consistent
		# even if the process is killed mid-run.
		try:
			with open(self._trace_collect_out, 'w', newline='') as f:
				w = _csv.writer(f)
				w.writerow(['call_idx', 'tau_ms'])
				for call_idx, tau_ms in self._trace_collect_rows:
					w.writerow([call_idx, '%.4f' % tau_ms])
		except Exception as ex:
			print('[pnp_infer] trace_collect flush failed:', ex)
	...
	def get_action_from_list_inter(self, car_data_raw, rsu_data_raw, step, timestamp):
		...
		## communication latency, get data from N frames past (legacy static mode)
		...
		## ★ ego 推理延迟注入 (τ_perc): 延迟 ego 自身完整输入 Δ_perc 帧 + ZOH
		## 物理: 端到端推理延迟 → ego 在第 step 帧基于第 (step-Δ) 帧的世界感知行动
		## (与 comm_latency 同模式但作用于 ego i==0; 协同/RSU 分支不变)
		## 存/取深拷贝: preprocess_data 原地翻转点云 + ZOH 重复取用会污染 bank
		if getattr(self, '_tau_perc_ms', 0.0) > 0 and car_data_raw and car_data_raw[0] is not None:
			delta_p = min(int(math.ceil(self._tau_perc_ms / 50.0)), 20)  # cap 20帧=1000ms (支持tp1000)
			self._ego_perc_bank[step] = copy.deepcopy(car_data_raw[0])
			_avail = [k for k in self._ego_perc_bank if k <= step - delta_p]
			if _avail:
				_uk = max(_avail)
				car_data_raw[0] = copy.deepcopy(self._ego_perc_bank[_uk])
				if step % 40 == 0:
					print("[pnp_infer] ★ τ_perc=%.0fms Δ_perc=%d step=%d → ego感知@frame%d (滞后%d帧)" % (self._tau_perc_ms, delta_p, step, _uk, step-_uk))
			for _k in [kk for kk in self._ego_perc_bank if kk < step - delta_p - 3]:
				del self._ego_perc_bank[_k]

		## ★ latency-aware dynamic frame alignment + ZOH (design §3.2/§3.3)
		## Replaces the static comm_latency with a per-frame measured/injected
		## latency -> Delta = ceil(L/50ms) delay applied to the RSU/other-CAV
		## branch only; ego self-data (i==0) stays current. ZOH on the RSU path.
		if self._lat_aligner is not None:
			# producer: submit this frame's RSU+collab payload, keyed by frame id
			rsu_payload = {
				'car_data': [car_data_raw[i] if i > 0 else None
							 for i in range(len(car_data_raw))],
				'rsu_data': rsu_data_raw,
			}
			# ★ R-3: per-call tau from trace (cycles when exhausted), or constant inject
			if self._trace_reader is not None:
				lat_ms = self._trace_reader.next()
			else:
				lat_ms = self._latency_inject_ms  # injected E7 archive (or None -> measured wiring in C-4+)
			delta_sub = self._lat_aligner.submit(
				step, rsu_payload, lat_ms, self._latency_source_tag)
			# consumer: pick the delayed (and possibly zero-order-held) RSU result
			used_payload, audit = self._lat_aligner.consume(step)
			self._lat_aligner.gc(step)
			if used_payload is not None:
				for i in range(len(car_data_raw)):
					if i > 0 and i < len(used_payload['car_data']) \
							and used_payload['car_data'][i] is not None:
						car_data_raw[i] = used_payload['car_data'][i]
				rsu_data_raw = used_payload['rsu_data']
			# else: no RSU available yet at sim start -> ego keeps its own current
			#       perception (caller contract; ego never delayed/held), design §3.3-3
			self._lat_audit_rows.append(audit)
			self._dump_latency_audit()
		...
		# ── §cross-machine mean-shift payload capture ─────────────────────────
		# Set PAYLOAD_CAPTURE_DIR=/path/to/dir to capture ONE corridor frame.
		# Capture criteria: lidar_pose_y ∈ [CAPTURE_Y_MIN, CAPTURE_Y_MAX] and
		#   |lidar_pose_x - CAPTURE_X_CENTER| < CAPTURE_X_RANGE.
		# Saved as pickle: {'extra_source': ..., 'step': int, 'ego_x': float,
		#   'ego_y': float, 'machine': str}
		# Use b_test_payload_replay.py to replay on another machine.
		_cap_dir = os.environ.get('PAYLOAD_CAPTURE_DIR', '')
		_cap_max = int(os.environ.get('CAPTURE_MAX_FRAMES', '5'))
		if _cap_dir and getattr(self, '_payload_capture_count', 0) < _cap_max:
			try:
				_m0 = car_data_raw[0]['measurements'] if car_data_raw and car_data_raw[0] else {}
				_ego_y = float(_m0.get('lidar_pose_y', 999))
				_ego_x = float(_m0.get('lidar_pose_x', 999))
				_cap_y_min = float(os.environ.get('CAPTURE_Y_MIN', '-70'))
				_cap_y_max = float(os.environ.get('CAPTURE_Y_MAX', '-44'))
				_cap_x_ctr = float(os.environ.get('CAPTURE_X_CENTER', '-44.3'))
				_cap_x_rng = float(os.environ.get('CAPTURE_X_RANGE', '12'))
				if (_cap_y_min <= _ego_y <= _cap_y_max and
						abs(_ego_x - _cap_x_ctr) <= _cap_x_rng):
					...
					_cap_payload = {
						...
						'note': 'captured by pnp_infer_action_e2e.py get_action_from_list_inter',
					}
					_cap_fname = os.path.join(_cap_dir,
						'corridor_payload_step%04d_x%.1f_y%.1f.pkl' % (step, _ego_x, _ego_y))
					with open(_cap_fname, 'wb') as _cf:
						pickle.dump(_cap_payload, _cf, protocol=4)
					print('[PAYLOAD_CAPTURE] saved %s (ego_x=%.1f ego_y=%.1f step=%d)'
						  % (_cap_fname, _ego_x, _ego_y, step), flush=True)
					self._payload_capture_count = getattr(self, '_payload_capture_count', 0) + 1
			except Exception as _cap_ex:
				print('[PAYLOAD_CAPTURE] error (non-fatal):', _cap_ex, flush=True)
		# ──────────────────────────────────────────────────────────────────────
		...
		# ── §3.8 R-1 τ-trace collection: CUDA-Event start ────────────────────
		if self._trace_collect_out is not None:
			if self._tc_cuda:
				torch.cuda.synchronize()
				self._tc_evt_s.record()
			else:
				_tc_t0 = time.time()
		# ──────────────────────────────────────────────────────────────────────
		...
		# ── §3.8 R-1 τ-trace collection: CUDA-Event stop ─────────────────────
		if self._trace_collect_out is not None:
			if self._tc_cuda:
				self._tc_evt_e.record()
				torch.cuda.synchronize()
				tau_ms = self._tc_evt_s.elapsed_time(self._tc_evt_e)
			else:
				tau_ms = (time.time() - _tc_t0) * 1000.0
			self._trace_collect_calls += 1
			if self._trace_collect_calls > self._trace_collect_warmup:
				call_idx = self._trace_collect_calls - self._trace_collect_warmup - 1
				self._trace_collect_rows.append((call_idx, tau_ms))
				self._flush_trace_collect()
		# ──────────────────────────────────────────────────────────────────────
	...
	def generate_action_from_model_output(self, pred_waypoints_total, car_data_raw, rsu_data_raw, car_data, rsu_data, batch_data, planning_input, car_mask, step, timestamp):
		...
		for count_i in range(self.ego_vehicles_num):
			...
			# ★ I-1 L1: anchor inference-frame ego-local waypoints to GPS-metric world frame.
			# Stored in L1PlanStore; replayed at every tick in run_l1_step() via world→local.
			# measurements['theta'] is raw compass; l1_trajectory_controller uses theta+π/2 internally.
			if self._l1_plan_stores is not None:
				_m = car_data_raw[ego_i]['measurements']
				self._l1_plan_stores[ego_i].update(
					pred_waypoints,                       # (10, 2) ego-local BEV frame
					_m['gps_x'], _m['gps_y'],
					_m['theta'],                          # compass (radians)
					timestamp,                            # CARLA game time at this inference frame
					self.config['control']['max_speed'],  # ★ V2: freeze desired_speed
				)
			...
			# compat: speed/target may be float/scalar (torch2) or tensor (torch1)
			route_info['speed'] = route_info['speed'].tolist() if hasattr(route_info['speed'], 'tolist') else float(route_info['speed'])
			route_info['target'] = route_info['target'].tolist() if hasattr(route_info['target'], 'tolist') else list(route_info['target'])
	...
	# ── ★ I-1 L1 trajectory tracking step (non-inference frames) ─────────────────

	def run_l1_step(self, ego_data, timestamp):
		"""L1 control for non-inference frames when l1_control=true.

		Called each tick between inference frames.  Uses the world-frame plan
		anchored at the last inference frame (L1PlanStore) to compute a fresh
		control command that accounts for plan age (time-indexed desired_speed).

		Parameters
		----------
		ego_data : list[dict|None]
			Per-vehicle tick() output from pnp_agent_e2e (fresh pose + target_point).
		timestamp : float
			CARLA game time (seconds), same source as generate_action_from_model_output.

		Returns
		-------
		dict {vehicle_idx: carla.VehicleControl}  — same shape as prev_control.
		Empty dict if all vehicles have no plan yet (→ caller falls back to prev_control).

		口径: I-1 / L1 / non-inference frame; no GPU, no model forward pass.
		"""
		from team_code.closedloop.l1_trajectory_controller import N_WP
		import numpy as _np_l1
		control_out = {}
		ego_i = -1
		for count_i in range(self.ego_vehicles_num):
			if ego_data[count_i] is None:
				continue
			ego_i += 1
			store = self._l1_plan_stores[ego_i]
			# ★ I-2: select the τ_ego-delayed active plan for the current sim time.
			# τ_ego=0 → newest plan immediately active (== I-1 baseline).
			# τ_ego>0 → plans within the last Δ_ego frames are not yet available,
			#           so an older plan is held (ego reacts Δ_ego frames late).
			store.select_active(timestamp)
			if not store.has_plan:
				# No active plan yet (sim start / τ_ego not elapsed) → L0 fallback
				continue

			m      = ego_data[count_i]['measurements']
			speed  = m['speed']
			target = m['target_point']   # fresh global-route target → steering

			# ★ V2: frozen desired_speed from inference frame (L0-proven formula, no geometric spike)
			# Synthetic straight-ahead waypoints make the controller recover v_frozen exactly
			# via mean(diff(norms(wp))[:3]) * max_speed.  Steering uses target (weight=1).
			v_frozen  = store.desired_speed
			_max_spd  = self.config['control']['max_speed']
			_spacing  = v_frozen / _max_spd if _max_spd > 0 else 0.0
			wp_synth  = _np_l1.column_stack([
				_np_l1.zeros(N_WP),
				-_spacing * _np_l1.arange(N_WP),
			])  # norms=[0,s,2s,...] → diffs=[s,...] → mean*max_speed = v_frozen

			# ★ L1-DIAG (optional trace, gated by simulation.l1_debug)
			_l1_debug = self.config.get('simulation', {}).get('l1_debug', False)
			if _l1_debug:
				_dt = max(timestamp - store.active_t_anchor, 0.0)
				print(
					f"[L1DBG] t={timestamp:.2f}s dt={_dt:.3f}s "
					f"tau_ego={getattr(self,'_tau_ego_ms',0.0):.0f}ms "
					f"v_frozen={v_frozen:.3f}m/s ego_spd={speed:.2f}m/s "
					f"spacing={_spacing:.4f}",
					flush=True
				)

			route_info = {
				'speed':        speed,
				'waypoints':    wp_synth,  # ★ V2: synthetic → encodes v_frozen; steering from target
				'target':       target,    # fresh → steering tracks current route (weight=1)
				'route_length': 0,
				'route_time':   0,
				'drive_length': 0,
				'drive_time':   0,
			}
			# ★ Fix-D: use ISOLATED L1 controller (not shared inference controller)
			# self.controller[ego_i] is only touched on inference frames (5Hz); fully protected.
			steer, throttle, brake, _ = self._l1_controllers[ego_i].run_step(route_info)
			if brake < 0.05:
				brake = 0.0
			if brake > 0.1:
				throttle = 0.0
