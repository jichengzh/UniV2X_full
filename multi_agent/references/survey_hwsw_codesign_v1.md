...
**为什么必须协同 — 两类理由四句话** [v1.4 修订: 应用户指正补"进取性"理由]:

*防御性理由(保证软件优化在硬件上真正生效)*:
...
*进取性理由(到达单侧不可达的 Pareto 边界)*:
4. **联合搜索空间的 Pareto 边界严格优于任何单侧优化的边界**: 形式上, 联合空间 S×H ⊋ (单软件侧 S×{h₀}) ∪ (单硬件侧 {s₀}×H), 其 Pareto 前沿弱支配两个单侧前沿, 且实证为**严格支配**:
   - [我方实证] **单硬件侧最优(TRT 编译+FP32, 不动模型)**: Orin body 131.33ms / e2e 合成 ~219ms — **不进 200ms 可行域**; **单软件侧(eager 剪枝/量化, 不上编译器)**: 未实测 — 已测锚=未剪枝 eager 260.61ms, eager 不可剪段(vox+enc+bb+head+NMS)≈117.8ms 实测构成下限且 eager 无 INT8 加速路径, 进入 200ms 可行域无已验证路径 [推断: 剪枝后 eager 未实测]; **联合(剪枝×INT8×TRT×精度路由)**: body 20.02ms / e2e ~108ms — **唯一已验证进入可行域的方案**, 且其 (latency, AP, energy) 点不被任何单侧配置支配（数据: E7/E8 + `orin_pyramid_baseline_report_v1.md §4.2`, 均经核验）。
   - [文献声称] APQ: 同延迟约束 +2.3% 精度 = 联合前沿点支配串行组合前沿点（arXiv:2006.08509）; OFA: 设备专属子网支配通用模型（+4% acc **且** 1.5× faster, 双指标同时占优 = 严格 Pareto 支配, arXiv:1908.09791）; HAWQv3: 混精 INT4/INT8 联合整数规划比均匀 INT8 **最高 50% 加速**且精度持平（arXiv:2011.10680）。
   - 直观机理: 单侧优化只能沿自己的轴移动工作点, 而延迟/能耗等目标是 (软件结构 × 硬件路径) 的**联合函数**(见 §1.5 P1: 算术强度 I 由两侧共同决定) — 只有联合移动才能到达目标函数的真实最优等高线。
...
| **Pareto 边界拓展（我方, 进取性）** | 单硬件侧(TRT FP32) e2e ~219ms ❌ / 单软件侧(eager 剪枝/量化, 不上编译器): **未实测** — 已测锚=未剪枝 eager 260.61ms, eager 不可剪段 117.8ms 实测构成下限且 eager 无 INT8 加速路径, 进入 200ms 可行域无已验证路径 [推断] / **联合 ~108ms ✅ 唯一已验证进入 200ms 可行域** | [我方实证+推断] 因果链第 4 条; E7/E8 + orin_pyramid_baseline_report_v1.md §4.2 |
