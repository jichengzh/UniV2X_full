...
## 0. 一句话现状
HEAL PyramidFusion 已移植进 V2Xverse、在其 CARLA 数据上训练(感知40ep AP@0.5车0.91 + 专属planner 2ep)、接入闭环驾驶、与 CoDriving 做了**公平 τ=0 对比**(Pyramid自训planner DS74.6/RC100 vs CoDriving DS85.3/RC90.3)。全程隔离在 H800 `/exdata/jichengzhi/V2Xverse_pyramid`，**仿真主框架/t2lib 零污染**。

## 1. 环境与硬约束 (★必守)
- **H800 SSH**: `sshpass -p '12345678' ssh -p 30001 jichengzhi@222.95.84.215` (密码每会话确认)。
- **隔离副本(所有工作只在这)**: `/exdata/jichengzhi/V2Xverse_pyramid` (从sim副本rsync, 含闭环代码+Pyramid opencood+训练产物)。
...
- **数据集**: 163GB HF gjliu/V2Xverse(仅weather-0/96route)下到 `/exdata/jichengzhi/hf_cache`, 解压到 `/exdata/jichengzhi/v2xverse_dataset/weather-0/data/<route>/`, 索引 dataset_index.txt(绝对路径)。实际可用 **83 完整route**(4条下载中断残缺已排除)。
...
为公平: V2Xverse协议是**每感知方法单独训planner**(感知冻结, train_planner_e2e.sh)。给Pyramid训了专属planner(end2end_pyramid.yaml=copy codriving版weathers改[0], 2epoch loss0.022收敛, ckpt `.../planner_e2e-zs-nj-tap-gpu18-20260614-093449/models/epoch_1.ckpt`)。eval配置 `pnp_config_pyramid_eval.yaml`(planner指向自训ckpt)。
...
## 5. 关键命令速查
- **起CARLA(GPU5)**: `bash /data/jichengzhi_v2x/h800_carla_launch.sh 5 <port>` (port用全新如4400避TM残留)。等 `nc -z 127.0.0.1 <port>`。
- **起process B(感知server)**: `cd /exdata/jichengzhi/V2Xverse_pyramid; CUDA_VISIBLE_DEVICES=5 PYTHONPATH=/data/jichengzhi_v2x/t2lib:.:simulation/leaderboard setsid nohup python3 simulation/leaderboard/team_code/closedloop/process_b_server.py --port 5557 --gpu 5 --pnp-config <cfg> >log 2>&1 </dev/null &` 等 PROCESS_B_READY。
...
- **对比sweep脚本范本**: `/exdata/jichengzhi/sweep_fair2.sh`(端口4400, process B切换+route循环, 可改route集/config)。

## 6. 踩过的坑 (省时间)
- **sshpass长命令stdout易截断**; setsid启长任务会让SSH挂2min(正常,进程已起,另开命令核验)。写文件用scp/base64不用heredoc。
