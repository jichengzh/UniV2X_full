...
## 5. 环境/路径/产物(接手用)
- **H800**(数据+训练+导出): `sshpass -p 12345678 ssh -p 30001 jichengzhi@222.95.84.215`(密码每会话确认)。隔离副本 `/exdata/jichengzhi/V2Xverse_pyramid`, env `PYTHONPATH=/exdata/jichengzhi/tp_lib:/data/jichengzhi_v2x/t2lib:. python3`(torch2.1.2, **torch_pruning 1.6.0 隔离装在 tp_lib, 勿改 t2lib**)。
- **4090**(TRT+协同AP harness): env `/home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python`, TRT10.13。**DAIR 数据已传 4090** `/data/jichengzhi_dair/dair_eval/`(48G)。4090 V2Xverse opencood 已编 box_overlaps py39。
- **关键脚本**:
  - 剪枝: H800 `/exdata/jichengzhi/V2Xverse_pyramid/depgraph_codriving.py`(`--ratio R --out-dir ...`)
...
CUDA_VISIBLE_DEVICES=6 PYTHONPATH=/exdata/jichengzhi/tp_lib:/data/jichengzhi_v2x/t2lib:. \
