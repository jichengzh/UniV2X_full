...
log: ${V2X_DATA_ROOT}/v2x_t1_attention/logs/attention_e2e_pq_v1/hmsa_type_dispatch_coverage_full_v1.json
status: OK
sample_count: 1789
...
```text
ssh host: ${V2X_REMOTE_USER}@<PRIVATE_HOST> -p 30001
...
```text
TVM env: ${V2X_DATA_ROOT}/tvm310/bin/python
...
```text
${V2X_DATA_ROOT}/v2x_t1_attention/checkpoints/HeterBaseline_DAIR_lidar_v2xvit_2023_09_09_11_19_26/config.yaml
${V2X_DATA_ROOT}/v2x_t1_attention/checkpoints/HeterBaseline_DAIR_lidar_v2xvit_2023_09_09_11_19_26/net_epoch_bestval_at17.pth
${V2X_DATA_ROOT}/v2x_t1_attention/models/v2xvit_attention_t1/attention_p50_shortft_steps100_lr0.0001_seed20260623.pth
${V2X_DATA_ROOT}/v2x_t1_attention/models/v2xvit_attention_t1/attention_p50_shortft_steps100_manifest_v1.json
```

安全连接方式：

```bash
# 不要把密码写进命令、文档或最终汇报。用 password-based SSH (disabled; use an SSH key) 环境变量。
export - <<'PY'
from pathlib import Path
import re
text = Path('multi_agent/methods/design/plan_transformer_into_framework_v1.md').read_text(encoding='utf-8')
match = re.search(r"ssh\s+-p\s+30001", text)
if not match:
    raise SystemExit('missing H800 password pattern')
print(match.group(1))
PY
)"
ssh -p 30001 -o StrictHostKeyChecking=accept-new ${V2X_REMOTE_USER}@<PRIVATE_HOST> 'hostname'
```
...
rsync -av -e 'ssh -p 30001 -o StrictHostKeyChecking=accept-new' \
  ...
  ${V2X_REMOTE_USER}@<PRIVATE_HOST>:${V2X_DATA_ROOT}/v2x_t1_attention/scripts/phase2/
```
