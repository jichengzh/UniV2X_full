...
- **核查**: `pgrep -f eval_tp_errors_corrected_full` + `tail logs/eval_tp_corrected_full_gpu7.log`。
  - 已完成(csv 在) → 直接走 §二 收口链。
  - 还在跑 → 等(完成前先拉团队 §三)。
  - 死了 → 重启: `CUDA_VISIBLE_DEVICES=<空闲卡> nohup ${V2X_HOME}/miniconda3/envs/UniV2X_2.0/bin/python scripts/phase2/eval_tp_errors_corrected_full.py > logs/eval_tp_corrected_full.log 2>&1 &`(跑前 `nvidia-smi` 确认 0%util/≤50MiB; 06-04 时 0/1/2/7 空闲, 3/4/5/6 被他人占)。
...
## 九、事实源
`team_charter_v1.md` · `background/00_研究目标与实验档案_v1.md` · `issues_log_v1.md`(ISS-001~028) · `NEXT_PHASE_plan_v1.md` · `pareto_definition_v1.md §七` · `data/schema_v2.md` · 本文件。Orin: ${V2X_REMOTE_USER}@<PRIVATE_HOST>(密码每 session 向用户确认)。
