...
- **★关于"gpu-reset 不可用"的纠正**: 旧 handoff 写的 "Fabric Manager 持卡→gpu-reset 必失败" **是误判**。真相: **per-GPU reset 只在该卡有 compute 进程持着时失败**(报 "In use by another client" 指的是那个进程, 不是 FM)。**卡上无 compute 进程时, `sudo nvidia-smi --gpu-reset -i N` 能成功**(本次 5/6/7 都成了; GPU4 因有 <LOCAL_USER> python 持着才 reset 不了)。⇒ **恢复流程 = ①先 kill/挪走该卡上的 compute 进程 ②`sudo nvidia-smi --gpu-reset -i N`(需 sudo, 用户执行) ③重测单 CARLA 确认 PORT-UP**。
- **验证旁证**: 同一条 `-opengl -RenderOffScreen` 命令 + 同 CARLA, 在**本地 4090(<PRIVATE_HOST>)上能正常 PORT-UP**(GPU6 显存 1648→2381MiB), 只有 H800 wedge → 坐实是 H800 节点 GPU 状态问题, reset 即解。
- **4090 fallback(备用, 本次未走)**: 本地 `${V2X_ROOT}verse` 有完整 CARLA+CoDriving 权重+`v2xverse`/`t2val` env, CARLA 渲染实测正常; 但 closedloop 是**旧快照**(只有 `infer_server.py`, 无 `process_b_server.py`/AP 注入/t2lib), 迁移需从 H800 传 isolated 副本+t2lib, 且 8 卡中仅 GPU6 空。仅当 H800 reset 也救不了时才考虑。
- **重测命令**(确认某 GPU 是否恢复, 在 GPU 空闲时):
  ...
  # 等 ~100s; grep "re-raising signal" 该log=还崩; nc -z <PRIVATE_HOST> 7700=恢复
  ```
...
- **路集**: 8 条 tp0-clean (`${V2X_DATA_ROOT}/grid_routes.txt` = 3,17,18,104,136,312,317,324), N=3, 诚实DS(超时→0), full-traffic `_1`。
...
结果路径 `${V2X_DATA_ROOT}/V2Xverse_apknob/results/results_driving_grid_g{code}_r{R}_n{N}/v2x_final/town05_short_collab/r{R}_repeat0/ego_vehicle_0/results.json`。
...
harness = `${V2X_DATA_ROOT}/tau_curve_apknob.py`(VXDIR/CARLA_L 已指副本+noadapter脚本)。
**已修**:
...
2. **CARLA `-graphicsadapter=$GPU` 在 GPU1 段错误**: 已改用无 graphicsadapter 的启动脚本 `${V2X_DATA_ROOT}/h800_carla_launch_noadapter.sh`(靠 CUDA_VISIBLE_DEVICES 隔离), harness CARLA_L 已指它。
...
3. **续 Step A 补缺口**(服务器端脚本避 ssh 截断):
   ...
   CFG_TAG=g TAUS=0,25,50,70,85 NREP=3 ROUTE_FILE=${V2X_DATA_ROOT}/grid_routes.txt \
     TAG_PREFIX=grid SCEN_SUFFIX=_1 GPULIST=0,1,2,3 SLOTS_PER_GPU=1 \
     PORT_BASE=4600 BPORT_BASE=6500 RUN_TIMEOUT=900 \
     setsid nohup python3 ${V2X_DATA_ROOT}/tau_curve_apknob.py > ${V2X_DATA_ROOT}/grid_stepA.log 2>&1 </dev/null &
   ```
...
## 6. 关键资源
- **H800**: `ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(密码每会话确认; ssh 易 stall, 长任务用 setsid+服务器脚本)。
- **隔离副本**(所有 AP 工作, 主框架零污染): `${V2X_DATA_ROOT}/V2Xverse_apknob`。
- **harness**: `${V2X_DATA_ROOT}/tau_curve_apknob.py`; **CARLA 启动**: `${V2X_DATA_ROOT}/h800_carla_launch_noadapter.sh`。
- **路集**: `${V2X_DATA_ROOT}/grid_routes.txt`。
- **离线 AP 标定**: `V2Xverse_apknob/opencood/tools/inference_apcalib.py`(env APCALIB_TESTDIR=${V2X_DATA_ROOT}/v2xverse_dataset/ AP_DROP=<d> APCALIB_NMAX=200)。
