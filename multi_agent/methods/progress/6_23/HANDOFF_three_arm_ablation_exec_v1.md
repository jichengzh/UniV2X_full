...
## §7 资源 / 脚本 / 纪律
- **H800 TVM(主)**: `ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(密码 `12345678`, 代理7897; 每会话确认); env `${V2X_DATA_ROOT}/tvm310/bin/python`; 跑前 `export PATH=/usr/local/cuda-12.2/bin:$PATH; export LD_LIBRARY_PATH=$(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path); export CUDA_VISIBLE_DEVICES=0`。资产 `${V2X_DATA_ROOT}/s2_tvm/models/{base,p50,p75,trap25,trap25_pad64,iso_s0,iso_s1,iso_s2}_backbone.onnx`。
...
- **4090(边缘/finetune)**: env `${V2X_HOME}/miniconda3/envs/UniV2X_2.0/bin/python`; HEAL `${V2X_HOME}/heal_research/HEAL`; **真仓库 `${V2X_ROOT}`**(UniV2X 断裂符号链接)。ONNX 导出器 `tools/export_backbone_onnx_new_widths.py`(改 num_filters 出任意宽度 backbone 子图, 已验证)。
...
## §8 接手第一步(再强调)
1. 确认 H800 可达(`echo > /dev/tcp/<PRIVATE_HOST>/30001`)+ env。
...
- 新脚本: `${V2X_DATA_ROOT}/s2_tvm/s2_2e_bumped.py` (只改 builder timeout)

### 已完成
- ✅ **12 个新 ONNX 导出** (9 已有 + 3 新: mix_d=[48,128,128], mix_e=[64,96,128], mix_f=[32,64,64])
  所有在 `models/stage_a_cache/` 且已传 H800 `${V2X_DATA_ROOT}/s2_tvm/models/`
...
ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST> \
  'tail -10 ${V2X_DATA_ROOT}/s2_tvm/lut_grid.log && cat ${V2X_DATA_ROOT}/s2_tvm/lut_results_grid.csv'

# 2. scp
scp -P 30001 \
  ${V2X_REMOTE_USER}@<PRIVATE_HOST>:${V2X_DATA_ROOT}/s2_tvm/lut_results_grid.csv \
  ${V2X_ROOT}/results/lut_results_grid.csv
