...
**全部隔离在 `${V2X_DATA_ROOT}/V2Xverse_apknob`,仿真主框架 `/data/jichengzhi_v2x/V2Xverse` 零污染(git diff 已验空)**。
...
| 0.75 | 0.310 | 0.291 |
| 0.9 | 0.266 | 0.250 |
跑法: `CUDA_VISIBLE_DEVICES=N PYTHONPATH=/data/jichengzhi_v2x/t2lib:<COPY> APCALIB_TESTDIR=${V2X_DATA_ROOT}/v2xverse_dataset/ APCALIB_NMAX=200 AP_DROP=<d> python3 opencood/tools/inference_apcalib.py --model_dir checkpoints/codriving/perception --fusion_method intermediate`
...
- **副本**: `${V2X_DATA_ROOT}/V2Xverse_apknob`(rsync 自主框架 741M, 排除 results;含 AP 注入 agent + 4 个 ap configs + apcalib)。
- **harness**: `${V2X_DATA_ROOT}/tau_curve_apknob.py`(VXDIR/LOGDIR/PROGLOG 已指副本)。
- **跑闭环 AP sweep**(范本, 复用 tau harness 全部断点续跑/诚实DS/超时逻辑):
  ...
  CFG_TAG=ap TAUS=0,50,55,100 NREP=2 ROUTE_FILE=<routes.txt> \
    ...
    setsid nohup python3 ${V2X_DATA_ROOT}/tau_curve_apknob.py > apval_run.log 2>&1 < /dev/null &
  ```
