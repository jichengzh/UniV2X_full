...
6. **重启清理**: pkill tau_curve/CARLA/process_b/leaderboard → 确认 CARLA=0 + 端口空(`nc -z <PRIVATE_HOST> PORT`) → 再 launch; 改 config 用 sed **替换**非新增(te0 base 已含 latency_inject_ms:0)。
