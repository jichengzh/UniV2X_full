...
## §5 资源 / 平台 / 脚本
- **H800 TVM(主, 当前全空)**: `ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(密码每会话确认; 代理 7897); env `${V2X_DATA_ROOT}/tvm310/bin/python`(Unity 改版: tvm.s_tir.meta_schedule/dlight, get_sblock); 跑前 export PATH(cuda-12.2)+LD_LIBRARY_PATH(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path)+CUDA_VISIBLE_DEVICES。资产 `${V2X_DATA_ROOT}/s2_tvm/models/{base,p50,trap25}_backbone.onnx`(+codriving_cache)。
- **4090(本地, 边缘绝对)**: env `UniV2X_2.0`; TRT 10.13。**真仓库 `${V2X_ROOT}`**(UniV2X 是断裂符号链接)。
