...
## §1 连接 + 资源纪律
- SSH: `ssh -p 30001 -o StrictHostKeyChecking=accept-new ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(密码每会话确认)
...
for i in $(seq 1 90); do nc -z -w1 <PRIVATE_HOST> 3000 && { echo "CARLA ready"; break; }; sleep 1; done
```
...
- **pypi 不通** → 跑闭环不需联网; t2lib 已自足。要装新包才需代理 `--proxy http://<PRIVATE_HOST>:7890`。
