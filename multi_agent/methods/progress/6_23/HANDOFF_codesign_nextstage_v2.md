...
**仓库 / 环境(直接开工必备)**:
- 真仓库 = **`${V2X_ROOT}`**(⚠️ `${V2X_HOME}/UniV2X` 是断裂符号链接空壳, 别用)。
- 跑 framework 脚本(`run_b4_ablation.py`/`b5_verify_convergence.py`, 需 matplotlib/numpy/scipy)用 conda: **`${V2X_HOME}/miniconda3/envs/UniV2X_2.0/bin/python`**(系统 python3 缺 matplotlib 会报错)。
...
5. **TVM 环境正确启动**(直驱必备): `export LD_LIBRARY_PATH=$(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path)`; python `${V2X_DATA_ROOT}/tvm310/bin/python`; **CUDA codegen 还需 nvcc 在 PATH**(prior int8 work 有, 须定位并加入)。
...
- **第一步(解 blocker)**: 定位 **nvcc 加入 PATH** → 重跑 `q0_int8_spike.py`(GPU6, `LD_LIBRARY_PATH=$(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path)`)拿 **base TVM int8 真延迟 gate**(vs FP32 tuned 6320µs)。
...
- H800 TVM int8: `${V2X_DATA_ROOT}/s2_tvm/{q0_int8_spike.py, s2_2b_screenA_int8.csv, models/base_backbone.onnx}`; 环境 `LD_LIBRARY_PATH=$(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path)` + **nvcc on PATH**(待补)。
- H800 闭环: `${V2X_DATA_ROOT}/V2Xverse_pyramid`, eval GPU5, τ config `pnp_config_pyramid_sweep_d{0,108,219,500}.yaml`, 设计 `sim_closedloop_pyramid_ds_surface_v1.md`。
- 设计 `auto-tuning/4_design_ablation_proof_v1.md §9`(§9.8 = 本轮新结果)。
- H800 ssh: `ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(pw 12345678, password-based SSH (disabled; use an SSH key) ConnectTimeout≥45, 注意连太频会被 rate-limit)。
