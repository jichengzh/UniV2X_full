...
- **全量 te219 扫描**: 105 条全扫完(τ_ego=219ms)。分类(失败模式):
  - **撞行人(Scenario3, τ_ego 敏感) 23 条**: 3,11,12,14,15,16,17,18,21,25,104,136,151,152,312,317,318,320,321,324,327,328,330
  - 撞车(Scenario4) 23 条; 干净无信号 52 条; 未完成(太难) 13 条
- **te0 对照(已完成 16/23 行人路线配对)**: 
  - **★干净 τ_ego 信号池(te0 不撞→te219 撞行人) = 9 条**: **r3, 17, 18, 25, 104, 136, 312, 317, 324** ← 直接可用作 pilot 曲线的核心路线。
  - 两端都撞(te0 也撞=route 难, 非干净信号, 不入池) 7 条: r15,16,21,318,320,327,330。
  - 还缺 te0 对照 7 条(可补): r11,12,14,151,152,321,328(这些 te219 是 ped+veh 混合, 较脏)。
  - **干净率 ~56%**(16 条配对里 9 条干净) → 类型选库后仍需 te0 baseline 过滤掉"τ_ego=0 就崩"的路线(无动态范围)。
- **配置**: `pnp_config_codriving_te{0,108,136,219}_l1.yaml` 已建(agent_config/)。

### 进行中 🔄
- te0 确认批: GPU3(port44000)+GPU0(port44100) 跑 r3,15,16,17,18,312 / 317,318,320,324,327。log: `results/te_confirm/te0c_g{3,0}_*.log`。waiter: 见下次 ScheduleWakeup。

### 待做 (新窗口) ⏸
1. **P0 建库**: 按 `exp_design §1` 选 ~65 条(几何先验/场景密度), 冻结到 `scenario_library_v1.txt`。
