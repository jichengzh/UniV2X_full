...
| **CARLA <PRIVATE_HOST>** (16GB) | ❌ 不能进 git | §2-A `[需用户确认]` |
...
### A. CARLA <PRIVATE_HOST> (16GB) `[需用户确认]`
- 旧路径: `${V2X_ROOT}verse/carla/` (CarlaUE4 二进制 + AdditionalMaps)
...
- 旧路径: `${V2X_ROOT}verse/checkpoints/codriving/`
...
- 旧路径: `${V2X_HOME}/heal_research/checkpoints/stage1/`
...
### D. 数据集 `[需用户确认]`
- `${V2X_HOME}/heal_research/dataset/OPV2V_orig` (71GB) — HEAL 训练/AP eval 用。
- `${V2X_ROOT}verse/dataset/` (空, 4KB) — ★**闭环仿真不需要离线数据集**(用 CARLA 实时传感器), 空目录即可。
...
旧服务器 `${V2X_HOME}/miniconda3/envs/v2xverse`。完整重建(按 V2Xverse README + 本项目实测偏差):
```bash
...
pip install -r simulation/requirements.txt       # numpy 降到 1.18.3, opencv <PRIVATE_HOST>, py-trees 0.8.3 (含 setuptools==41 装 egg)
...
- 旧服务器 `${V2X_HOME}/miniconda3/envs/UniV2X_2.0`。
...
> 前提: §1.2 拿到 V2Xverse 代码 + §2 资产就位 + §3 环境建好。所有路径把 `${V2X_HOME}/` 换成新服务器实际路径。
...
# 等 CARLA 就绪 (冷启动可能 >90s, 用 nc -z <PRIVATE_HOST> 40000 探测; 别用 90s 死超时)
...
### 7.1 连接
```bash
ssh ${V2X_REMOTE_USER}@<PRIVATE_HOST> -p 30001     # host=<PRIVATE_HOST>
# 密码: ★用户口头提供, 每会话向用户确认, 勿写入文档/勿进 git
# 非交互(AI 用): ssh -p 30001 -o StrictHostKeyChecking=accept-new ${V2X_REMOTE_USER}@<PRIVATE_HOST> '<命令>'
```
...
| 磁盘 **`/data`** | 7T, **剩 269G**, `/data/<LOCAL_USER>` 可写 | ★**conda/CARLA/代码都装这** |

### 7.3 从零部署步骤 (★都装到 `/data/<LOCAL_USER>/`)
```bash
WORK=/data/<LOCAL_USER>
...
ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST> \
  'source /data/<LOCAL_USER>/miniconda3/etc/profile.d/conda.sh && conda activate v2xverse && cd /data/<LOCAL_USER>/V2Xverse && python simulation/leaderboard/team_code/closedloop/test_l1_trajectory_controller.py'
...
> - H800 上已部署的资产**保留不删**(env 8.5G 解包 + 3.7G tarball + V2Xverse 代码 503M, 在 `/data/<LOCAL_USER>/`), 将来若决定移植代码已就位; env 本身不可用(sm90)。
> - 下一步回到旧 4090 推进 **item4 I-1 L1 V1(Fix-D)验证**(§7.7), 推进 τ_ego。

### 7.7 旧服务器(4090, ${V2X_HOME})当前运行状态 — 新会话需知
