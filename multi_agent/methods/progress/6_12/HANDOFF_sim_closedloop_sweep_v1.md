...
1. **确认仿真进程是否还活着**(OS 级, 独立于窗口):
   ...
   ps aux | grep -E "world-port|closedloop_sweep|eval_driving_e2e" | grep -v grep | grep <LOCAL_USER>
   ls -d ${V2X_ROOT}/results/results_driving_lat_* 2>/dev/null   # 已完成几档
   ```
...
- conda env **`v2xverse`**(Python 3.7.16) + CARLA **<PRIVATE_HOST>** + CoDriving 官方 ckpt 全装齐, `import torch/carla/spconv/opencood` 全通。
...
> 结果落 **`${V2X_ROOT}verse/results/`**(脚本从 V2Xverse repo 跑, 相对 results/ 在那边, **不是 V2X/results/**): `results_driving_lat_{d0,d50,d108,d150,d219,d300,d500,norsu}/` + 聚合 `closedloop_sweep_v1.csv`。team-lead 已读 raw json + CSV 核验。
...
| 资源 | 路径 |
|------|------|
| **真仓库** | `${V2X_ROOT}`(UniV2X 是断裂符号链接空壳, 不读不写) |
| V2Xverse repo | `${V2X_ROOT}verse`(分支 **sim-closedloop**, 不污染 main) |
| conda env | `${V2X_HOME}/miniconda3/envs/v2xverse`(Python 3.7) |
...
| Orin(P2 用) | `<PRIVATE_HOST>` 用户 <LOCAL_USER>(密码每会话向用户确认), TRT <PRIVATE_HOST> |
