...
| 项 | 值 |
|---|---|
| SSH | `ssh jichengzhi@222.95.84.215 -p 30001` 密码 `12345678`（每次会话确认） |
...
| **内网隔离** | H800 **无外网**，pip/conda/apt 直连全失败。**必须走代理 `http://127.0.0.1:7890`**（Clash；`17890` 是坏的别用）。`pip3 install --proxy http://127.0.0.1:7890 ...` |
...
> `pip3 install --proxy http://127.0.0.1:7890 --target=/data/jichengzhi_v2x/t2lib --upgrade --force-reinstall --no-deps "numpy==1.26.4"`
...
代码在 `/data/jichengzhi_v2x/HEAL`（从本地 `/home/jichengzhi/heal_research/HEAL` rsync，排除了 logs/.git/*.pth）。
...
- ⚠️ **AP 可信度警告（务必处理再报 AP）**：`cooperative/label_world_backup` 和 `vehicle-side/label/lidar_backup` 当前是**软链到原始标签**（`label_world`/`lidar`）的 **front≈all 近似**，不是 HEAL 训练用的补全标注。**要拿能写论文的 AP，必须配正确的补全标注**：本地 `/home/jichengzhi/heal_research/dataset/my_dair_v2x_supplement/DAIR-V2X-C_Complemented_Anno/`（231M，未传），里面 `new_labels/{cooperative_label,vehicle-side_label}/label_world/` 才是真 backup 标签源。
...
### 5.4 相关文件（仿真侧，都在本地 `/home/jichengzhi/V2Xverse`，尚未传 H800）
...
```bash
# 连服务器
sshpass -p '12345678' ssh -p 30001 -o StrictHostKeyChecking=no jichengzhi@222.95.84.215
# 经代理装包（永远 --target，永远 --proxy）
/usr/bin/pip3 install --proxy http://127.0.0.1:7890 --target=/data/jichengzhi_v2x/t2lib <pkg>
# 跑 HEAL（torch2）
...
# 本地→H800 传文件
sshpass -p '12345678' rsync -e "ssh -p 30001 -o StrictHostKeyChecking=no" -a <src> jichengzhi@222.95.84.215:/data/jichengzhi_v2x/<dst>
# 起 CARLA(无头, GPU6)
