...
- **repo**: `${V2X_ROOT}verse` (branch `feature/l1-trajectory-tracker`); 主仓 `${V2X_ROOT}`。
- **跑实验脚本**: `scripts/te_multiroute_sc.sh <gpu> <port> <tau> "<route_ids>"` — 单卡扫一组路线一个 τ_ego 档。已修 unbound-var bug。
- **conda**: `${V2X_HOME}/miniconda3/envs/v2xverse/bin/python` (py3.7)。
- **CARLA**: `external_paths/carla_root/CarlaUE4.sh` (<PRIVATE_HOST>)。
...
```bash
cd ${V2X_ROOT}verse/simulation/leaderboard/team_code/agent_config
