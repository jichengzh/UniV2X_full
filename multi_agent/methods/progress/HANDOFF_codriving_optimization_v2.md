...
## 5. 环境/路径/脚本
- **4090**(TRT + 全网/profiler 实测): env `/home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python`, TRT10.13。DAIR 数据 `/data/jichengzhi_dair/dair_eval/`(48G, split json 在 `dair_eval/split_json/`)。V2Xverse 副本 `/home/jichengzhi/V2Xverse`(opencood, 已编 box_overlaps py39)。
- **H800**(训练/剪枝/导出): `sshpass -p 12345678 ssh -p 30001 jichengzhi@222.95.84.215`(密码每会话确认)。隔离副本 `/exdata/jichengzhi/V2Xverse_pyramid`, `PYTHONPATH=/exdata/jichengzhi/tp_lib:/data/jichengzhi_v2x/t2lib:.`。
...
- **模型 forward**: `/home/jichengzhi/V2Xverse/opencood/models/center_point_codriving.py`(`generate_predicted_boxes` = decode 待修处; backbone 多尺度在 `fusion_net` 里复用一次)。
