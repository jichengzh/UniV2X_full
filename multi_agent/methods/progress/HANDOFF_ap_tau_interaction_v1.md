...
- **路集**: clean 6 路 `/exdata/jichengzhi/grid_routes_clean6.txt` = [3,17,18,104,136,317]。
...
# 验: nc -z 127.0.0.1 78xx = PORT-UP; grep "re-raising signal" /tmp/ct_g$g.log = 还崩
...
export NREP=6 ROUTE_FILE=/exdata/jichengzhi/grid_routes_clean6.txt
...
setsid nohup python3 /exdata/jichengzhi/tau_curve_apknob.py > /exdata/jichengzhi/ix_run.log 2>&1 < /dev/null &
echo "harness pid $!"
```
...
## 6. 关键资源
- **H800**: `sshpass -p '12345678' ssh -p 30001 jichengzhi@222.95.84.215`(密码每会话确认; ssh 易 stall)。
- **隔离副本**(所有 AP 工作, 主框架零污染): `/exdata/jichengzhi/V2Xverse_apknob`。
- **harness**(已修 epid): `/exdata/jichengzhi/tau_curve_apknob.py`; **CARLA 启动**: `/exdata/jichengzhi/h800_carla_launch_noadapter.sh`(harness 内部用)。
- **路集**: `/exdata/jichengzhi/grid_routes_clean6.txt`。
- **配置生成器**: `/tmp/make_ix_configs.py`(生成 fine-τ configs; 若副本重建需重跑)。
- **进度日志**: `/exdata/jichengzhi/apknob_progress.log`; 结果 `V2Xverse_apknob/results/results_driving_grid_g{code}_r{R}_n{N}/...`。
