...
## §6 环境/路径/脚本 (两线共用)
- **H800** `ssh -p 30001 jichengzhi@222.95.84.215`(密码每会话确认; 代理 7897); GPU 常被外部 DDP 抢, 微秒 kernel 必空闲卡(mem≤50MiB)实测, 偶发竞争产 11–22µs 假尖峰(min-of-N 兜)。
- **Orin**(RSU 边缘卡)`ssh jichengzhi@172.16.62.222`(内网不走代理); torch 1.12+CUDA; Tegra 用 tegrastats 查 GR3D。
- **素材(H800 `/exdata/jichengzhi/s2_tvm/`)**: `models/{base,p50,trap25}_backbone.onnx`(Pyramid)+ `models/codriving_cache/{base,p50,p75}_backbone.onnx`(CoDriving)。
