...
```bash
sshpass -p '<H800_PASSWORD>' ssh -p 30001 -o StrictHostKeyChecking=no \
...
```bash
sshpass -p '<H800_PASSWORD>' scp -P 30001 -o StrictHostKeyChecking=no \
  jichengzhi@222.95.84.215:/exdata/jichengzhi/v2x_t1_attention/results/attention_axis_tvm_bench_v1.json \
  /home/jichengzhi/V2X/results/attention_axis_tvm_bench_v1.json
```
...
sshpass -p '<H800_PASSWORD>' scp -P 30001 -o StrictHostKeyChecking=no \
  scripts/phase2/t1_attention_tvm_bench.py \
  jichengzhi@222.95.84.215:/exdata/jichengzhi/v2x_t1_attention/scripts/t1_attention_tvm_bench.py

sshpass -p '<H800_PASSWORD>' ssh -p 30001 -o StrictHostKeyChecking=no \
  jichengzhi@222.95.84.215 'bash -s' <<'REMOTE'
set -o pipefail
export PATH=/usr/local/cuda-12.2/bin:$PATH
export LD_LIBRARY_PATH=$(cat /exdata/jichengzhi/tvm_nvlibs.path)
cd /exdata/jichengzhi/v2x_t1_attention
...
sshpass -p '<H800_PASSWORD>' scp -P 30001 -o StrictHostKeyChecking=no \
  jichengzhi@222.95.84.215:/exdata/jichengzhi/v2x_t1_attention/results/attention_full_tvm_bench_v3_static.json \
  /home/jichengzhi/V2X/results/attention_full_tvm_bench_v3_static.json
sshpass -p '<H800_PASSWORD>' scp -P 30001 -o StrictHostKeyChecking=no \
  jichengzhi@222.95.84.215:/exdata/jichengzhi/v2x_t1_attention/logs/attention_full_tvm_bench_v3_static.log \
  /home/jichengzhi/V2X/logs/attention_e2e_pq_v1/attention_full_tvm_bench_v3_static.log
```
...
```bash
/home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python - <<'PY'
import json
...
| `/home/jichengzhi/heal_research/HEAL/opencood/models/sub_modules/hmsa.py` | HMSA 实现参考 |
| `/home/jichengzhi/heal_research/HEAL/opencood/models/sub_modules/mswin.py` | MSwin/BWA 实现参考 |
...
- H800 TVM 必须用 fresh/明确环境: `PATH=/usr/local/cuda-12.2/bin:$PATH` + `LD_LIBRARY_PATH=$(cat /exdata/jichengzhi/tvm_nvlibs.path)`。
