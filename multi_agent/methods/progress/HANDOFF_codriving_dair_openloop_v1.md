...
## 3. 环境与路径 (★必守)
- **H800**: `sshpass -p 12345678 ssh -p 30001 jichengzhi@222.95.84.215`(密码每会话确认)。训练/评估都在 H800。
- **隔离副本(所有 codriving 工作在此)**: `/exdata/jichengzhi/V2Xverse_pyramid`(已含 codriving 模型+dair 适配器; 主框架 `/data/jichengzhi_v2x/V2Xverse` 零污染)。环境 **t2lib**: `PYTHONPATH=/data/jichengzhi_v2x/t2lib:. python3`(torch2.1)。
- **4090**: HEAL 在 `/home/jichengzhi/heal_research/HEAL`, env `UniV2X_2.0`(torch2.0); Pyramid ckpt `/home/jichengzhi/heal_research/checkpoints/stage1/Pyramid_DAIR_m1_base_2023_08_14_11_42_29`; DAIR 数据 `/data/lxf/DAIR-V2X`。另有 4090 隔离副本 `/data/jichengzhi_dair/V2Xverse_codriving`(最初建配置+smoke 处, 后切 H800)。
...
## 4. 关键产物
- **CoDriving DAIR 配置**: `/exdata/jichengzhi/V2Xverse_pyramid/opencood/hypes_yaml/dairv2x/lidar_only/dair_centerpoint_codriving.yaml`(从 dair_centerpoint_where2comm.yaml 改: 模型 `center_point_codriving`, fusion `ATTEN` feature_dim256, 范围对齐 HEAL 102.4×51.2, voxel [0.4,0.4,5], 单类车 anchor_number 1)。
...
- **mAOE 脚本**: `/exdata/jichengzhi/V2Xverse_pyramid/eval_tp_errors_codriving.py`(口径逐函数对齐 Pyramid `V2X/scripts/phase2/eval_tp_errors_corrected_full.py`); 结果 `<logdir>/tp_errors_result.json`。
- Pyramid 现场 live log(4090): `/home/jichengzhi/V2X/pyramid_dair_r51.log`。
...
CUDA_VISIBLE_DEVICES=4 PYTHONPATH=/home/jichengzhi/heal_research/HEAL \
  /home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python opencood/tools/inference.py \
