...
- **H800**(只 B5 补测才需):`ssh -p 30001 ${V2X_REMOTE_USER}@<PRIVATE_HOST>`(pw 12345678);env `export PATH=/usr/local/cuda-12.2/bin:$PATH; export LD_LIBRARY_PATH=$(cat ${V2X_DATA_ROOT}/tvm_nvlibs.path); export CUDA_VISIBLE_DEVICES=0`;tuner `${V2X_DATA_ROOT}/s2_tvm/s2_2e_bumped.py`(timeout=300 已修);python `${V2X_DATA_ROOT}/tvm310/bin/python`。**纪律**:fresh workdir 每点、idle GPU 测延迟、continue-on-error。
