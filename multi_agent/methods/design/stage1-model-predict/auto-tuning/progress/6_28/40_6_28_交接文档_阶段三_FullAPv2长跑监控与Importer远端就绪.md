# 46_6_28_交接文档_阶段三_FullAPv2长跑监控与Importer远端就绪
...
本轮核心进展: 继续监控 `s0_024` native INT8 full AP v2。v2 仍在 H800 上运行, 已推进到 `bridge_call_000363`, 当前无 sample blocker, full report 尚未写出。新增的 native INT8 AP row importer 已同步到 H800, 并通过远端两条关键测试。
...
本轮没有导入 native INT8 AP measured row。`native_int8_original60_ap_rows_v1.jsonl` 仍未生成/导入。
...
```text
time = 2026-06-28T08:49:10+08:00
elapsed = 00:29:59
process_state = running
gpu0_memory_used_MB = 13424
sample_blocker_count = 0
max_worker_tmp = 000363
...
```text
v2 已稳定越过 v1 sample_000125 blocker。
当前没有 sample_*_blocker.json。
full report 尚未写出, 不能执行 AP row import。
按当前 worker_tmp 推进速度, full 1789-sample eval 预计仍需要较长时间。
```

## 2. Native INT8 AP importer 已远端就绪
...
```text
${V2X_ROOT}/scripts/stage2_import_native_int8_full_ap_row.py
```

远端验证:

```text
PYTHONPATH=${V2X_ROOT} ${V2X_HOME}/miniconda3/envs/UniV2X_2.0/bin/python -m unittest \
  framework.tests.test_stage2_original60_quant_completion.Stage2Original60QuantCompletionTest.test_native_int8_full_ap_import_cli_converts_gated_report_to_compliant_ap_row \
  framework.tests.test_stage2_original60_quant_completion.Stage2Original60QuantCompletionTest.test_native_int8_full_ap_import_cli_rejects_smoke_report

Ran 2 tests in 0.320s
OK

PYTHONPATH=${V2X_ROOT} ${V2X_HOME}/miniconda3/envs/UniV2X_2.0/bin/python -m py_compile scripts/stage2_import_native_int8_full_ap_row.py
exit 0
```
...
如果 full report 出现并通过 gate, 在 H800 上运行:
