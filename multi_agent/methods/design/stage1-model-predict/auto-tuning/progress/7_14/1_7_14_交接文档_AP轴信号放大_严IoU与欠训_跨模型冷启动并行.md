...
**模型顺序(★用户拍板:CoDriving 先,F-Cooper/AttFuse 后):**
1. **CoDriving —— 首选,必须先做。** 理由:①它是除 Pyramid 外**最成熟**的模型,已有 TVM 迁移(`HANDOFF_codriving_tvm_migration_v1`)、真 int8 AP、剪枝×量化网格、iso-budget 分析(见 [[project-codriving-optimization-pilot]]/[[project-codriving-dair-openloop-ap]]),冷启动锚点能建在**真测数据**上,不必从零搭;②它恰是 **AP 轴有信号、且 int8×剪枝耦合是结构性的**模型(base ΔAP50 −0.003→p75 −0.062,~20× 放大,见 12_ §2.3)——与 Pyramid"解耦、无信号"正好对照,是验证框架能否迁到**不同耦合行为**模型的最佳载体;③measure/TVM 基础设施已就绪。
2. **F-Cooper / AttFuse —— 其次**(已 auto_scan 通,Phase A1,见 [[project-autoscan-phase-a0-a1]],但**尚无实测数据 = greenfield**)。作用 = 测"push-button onboard 全新模型"的冷启动路径是否成立。

**探针网格(每模型)**:P = 全模型统一剪枝率{0,0.25,0.5,0.75}(不分 stage)× Q = {fp32,fp16,int8_tc} → **12 anchor/模型**。
- 测量:复用 `framework/measure_config.py` 三精度接口(已 ready)+ §1 真 AP 口径(含真 int8);CoDriving 优先复用其已建 TVM backbone旋钮/int8 通路。
- 并入:训练表加 `model_class` one-hot + `is_coldstart_anchor` 标记;cost model 迁移/分层(Pyramid 全量 base + 目标 12 anchor 微调)。
- **判据**:留出目标模型若干分区配置作 test,比"零 anchor 直接迁"降 MAPE 多少 = 冷启动价值。**CoDriving 因有真测阶梯,可直接验证冷启动预测 vs 真值**。
- 顺带:whole-model 探针 = 三臂里 S0/serial 基线,给跨模型提供 argmin-drift 载体(不同模型 fp16-最优 vs int8-最优 全局压缩率漂移);CoDriving 的结构性耦合正好是最强 drift 载体。
- 坑:AttFuse/F-Cooper 的 fusion neck TVM 导入可能 blocked(见 unified handoff);attention 模型(V2X-ViT)瓶颈非 conv,现有 conv-TC rewrite 无效 → **本轮先做 CoDriving,再 conv-backbone 类(F-Cooper/AttFuse),V2X-ViT 留到 transformer 入框架计划**。
...
| C | **CoDriving 12-anchor 冷启动探针(★先做)** | GPU 3,4 | 复用已有 TVM/int8/剪枝阶梯 |
| C | F-Cooper / AttFuse 12-anchor 探针(其次,greenfield) | GPU 5,6 | auto_scan manifest |
...
```
/goal 阅读 multi_agent/methods/design/auto-tuning/progress/7_14/1_7_14_交接文档_AP轴信号放大_严IoU与欠训_跨模型冷启动并行.md 了解背景。本轮两大目标并行推进,所有实验真测(空闲GPU实测、区分真测/proxy、逐点核ckpt_path+n_samples、不信agent自报)。
...
目标二(跨模型冷启动,并行占另一批GPU):★模型顺序=CoDriving先(最成熟、有真测阶梯、int8×剪枝结构性耦合=与Pyramid对照),F-Cooper/AttFuse其次(greenfield测push-button onboard)。做12-anchor whole-model P×Q探针(P剪枝率{0,.25,.5,.75}×Q{fp32,fp16,int8_tc}),测lat/energy/真AP并入LUT,验证冷启动降MAPE价值。
...
- [ ] 目标二:CoDriving 12-anchor探针已测并入LUT(先做),report冷启动vs零anchor的MAPE降幅;有余力再做F-Cooper/AttFuse至少1个。
