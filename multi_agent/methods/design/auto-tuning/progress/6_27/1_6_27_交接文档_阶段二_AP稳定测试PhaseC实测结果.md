...
- `5_6_26_交接文档_阶段二_PhaseC_AP稳定产出规范.md`
- `RUNBOOK_stage2_h800_server_access_v1_zh.md`

## 0. 后续交接文档命名规则
从下一份文档开始，交接文档统一存储进入日期对应的文件夹，例如1_6_27_交接文档_阶段二_AP稳定测试PhaseC实测结果.md，存储进入6_27，如果进入新的一天，怎创建文件夹并将新的交接文档存入
...
7. 本地结果已从 H800 rsync 回 `/home/jichengzhi/V2X/multi_agent/data/stage2_lut_generation_v1/generated/ap_stability_20260626/`。
...
| HEAL | `/home/jichengzhi/heal_research/HEAL -> /exdata/jichengzhi/heal_research/HEAL` |
| checkpoint root | `/home/jichengzhi/heal_research/checkpoints -> /exdata/jichengzhi/heal_research/checkpoints` |
| UniV2X python | `/home/jichengzhi/miniconda3/envs/UniV2X_2.0/bin/python` |
| torch | 2.0.1+cu118 |
| numpy | 1.22.4 |
| opencood import | ok |
| AP preflight | pass |
...
```text
/goal 在 /home/jichengzhi/V2X 中继续 Stage2 AP stable smoke 后处理。先阅读 multi_agent/methods/design/auto-tuning/progress/1_6_27_交接文档_阶段二_AP稳定测试PhaseC实测结果.md 和 RUNBOOK_stage2_h800_server_access_v1_zh.md。目标: 对 s0_024 和 s1_048 做 second seed/repeat, 修复 s2_160 ONNX sanity fail, 并把 AP quality gate 明确为 first_seed_only / trend_anomaly_pending_repeat / repeat_passed。不要改写已有真实 AP 数值, 不要把 predicted/model-fit/interpolated AP 写成 measured row。stop condition: s0_024 和 s1_048 有 repeat 结果或明确 quarantine; s2_160 有 sanity 修复后的 AP row 或明确 export quarantine; original60 gap report 和 ap_stability_summary 更新并 rsync 回本地。
```
