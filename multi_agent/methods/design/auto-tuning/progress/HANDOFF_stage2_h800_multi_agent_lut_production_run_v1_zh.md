...
1. 本地 `SSHPASS` 和 `H800_PASSWORD` 环境变量均不存在。
...
2. planner 只能回退到 `/exdata/jichengzhi/s2_tvm/models/<label>_backbone.onnx` 和 `/exdata/jichengzhi/s2_tvm/workdirs/<label>`。
3. 本地没有 H800 `/exdata/jichengzhi/s2_tvm`, 因此本地只能得到 missing。
